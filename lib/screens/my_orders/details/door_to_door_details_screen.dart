...
import '../change_door_to_door_time.dart';
import 'boarding_pass_screen.dart';

class DoorToDoorDetailsScreen extends StatefulWidget {
  final int orderId;

  const DoorToDoorDetailsScreen({super.key, required this.orderId});

  @override
  State<DoorToDoorDetailsScreen> createState() =>
      _DoorToDoorDetailsScreenState();
}

class _DoorToDoorDetailsScreenState extends State<DoorToDoorDetailsScreen> {
      ...
      child: Column(
        children: [
          GenericOrderHeader(
            ...
            status: order.status,
            iconPath: 'assets/images/door_to_door_icon.png',
            fromLocation: order.from,
            toLocation: order.to,
            numberOfBags: order.numberOfBags.toString(),
            numOfPassengers: order.numberOfPassengers.toString(),
          ),
          const SizedBox(height: 20),
          AppointmentDetailsCard(
            sectionOneTitle: "PickUp",
            sectionOneDate: order.pickup?.date ?? '',
            sectionOneTime: order.pickup?.time ?? '',
            sectionTwoTitle: "Delivery",
            sectionTwoDate: order.delivery?.date ?? '',
            sectionTwoTime: order.delivery?.time ?? '',
            onChangeDate: () {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) =>
                      ChangeDoorToDoorTimeScreen(orderId: widget.orderId),
                ),
              );
            },
          ),
          ...
          OrderActions(
            showBoardingPass: order.hasBoardingPass,
            ...
            onBoardingPass: () {
              Navigator.push(
                ...
                MaterialPageRoute(
                  builder: (context) =>
                      BoardingPassScreen(orderId: widget.orderId),
                ),
              );
            },
          ),
